...
class Pelatihanmodel extends Purbamodel
{
    ...
    {
        $data = explode(" ", $id);
        $corequery = "SELECT COUNT(*) as totaln FROM datapelatihan WHERE kodeKerusakan='" . $data[1] . "'";
        $result = $this->konek->query($corequery);
        $result = mysqli_fetch_assoc($result);
        if ($result['totaln'] <= 1) {
            $querybobot = "DELETE FROM pembobotan WHERE `kodeKerusakan`='" . $data[1] . "'";
            $this->konek->query($querybobot);
        }
        echo "berhasil";
        $query = "DELETE FROM datapelatihan WHERE `id`=" . $data[0];
        return $this->konek->query($query);
    }
}
